...
he3d.t.blankImage = function(format, stype, width, height, color) {
	if (!color)
		color = [255.0, 0.0, 255.0, 255.0];
	switch (format) {
		case he3d.gl.RGB:
			var i = 0;
			if (stype == he3d.gl.FLOAT)
				var image = new Float32Array((height * width) * 3);
			else
...
he3d.t.load = function(texture) {
	...
	switch (texture.format) {
		...
		case 'depth':
			...
			break;
		case 'depthstencil':
			newtexture.format = he3d.gl.DEPTH_STENCIL;
			break;
		case 'rgb':
	...
	switch (newtexture.type) {
		...
		case 'canvas':
			...
			newtexture.image = newtexture.target.ctx.getImageData(
		...
		case 'image':
			...
			if (texture.internal)
				newtexture.image.src = he3d.path + '../textures/' + texture.filename;
			else
				newtexture.image.src = he3d.t.path+texture.filename;
			if (texture.name == undefined)
				newtexture.name = texture.filename.substring(0, texture.filename.lastIndexOf('.'));

			newtexture.image.onerror = function(e) {
				he3d.log("WARNING","Failed to load Image: " + this.src);
				newtexture.height = 64;
				newtexture.width = 64;
				newtexture.image = he3d.t.blankImage(he3d.gl.RGBA, he3d.gl.UNSIGNED_BYTE,
					newtexture.height,newtexture.width, [255.0, 0.0, 255.0, 255.0]);
...
he3d.t.store = function(tid) {
	...
	// Filtering
	if (t.mipmap) {
		if (t.filter.min != he3d.gl.LINEAR_MIPMAP_LINEAR &&
			t.filter.min != he3d.gl.NEAREST_MIPMAP_NEAREST) {

			if (t.filter.mag == he3d.gl.NEAREST) {
				t.filter.mag = he3d.gl.NEAREST;
				t.filter.min = he3d.gl.NEAREST_MIPMAP_NEAREST;
			} else {
	...
	if (!t.wrap[0])
		...
		he3d.gl.texParameteri(he3d.gl.TEXTURE_2D, he3d.gl.TEXTURE_WRAP_S, he3d.gl.REPEAT);

	if (!t.wrap[1])
		he3d.gl.texParameteri(he3d.gl.TEXTURE_2D, he3d.gl.TEXTURE_WRAP_T, he3d.gl.CLAMP_TO_EDGE);
	else
		he3d.gl.texParameteri(he3d.gl.TEXTURE_2D, he3d.gl.TEXTURE_WRAP_T, he3d.gl.REPEAT);

	t.loaded = true;

	he3d.gl.bindTexture(he3d.gl.TEXTURE_2D, null);
...
he3d.t.storeCubeMap = function(tid) {
	...
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_POSITIVE_X, 0,
		t.format, t.format, t.stype, t.images[0]);
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_NEGATIVE_X, 0,
		t.format, t.format, t.stype, t.images[1]);
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_POSITIVE_Y, 0,
		t.format, t.format, t.stype, t.images[2]);
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_NEGATIVE_Y, 0,
		t.format, t.format, t.stype, t.images[3]);
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_POSITIVE_Z, 0,
		t.format, t.format, t.stype, t.images[4]);
	he3d.gl.texImage2D(he3d.gl.TEXTURE_CUBE_MAP_NEGATIVE_Z, 0,
		t.format, t.format, t.stype, t.images[5]);
...
he3d.t.setFilterAnisotropic = function(level) {
	if (!he3d.t.af.ext || he3d.t.af.max == 0 ||
		he3d.t.af.level == level || level < 0 || level>he3d.t.af.max)
		return;
	...
	for (var tid = 0; tid < he3d.t.textures.length; tid++) {
		...
		he3d.gl.bindTexture(he3d.gl.TEXTURE_2D, he3d.t.textures[tid].texture);
		he3d.gl.texParameterf(he3d.gl.TEXTURE_2D,
			he3d.t.af.ext.TEXTURE_MAX_ANISOTROPY_EXT, he3d.t.af.level);
	}
...
he3d.t.viewer.draw = function () {
	...
	he3d.gl.viewport(
		he3d.gl.viewportWidth / 2 - width / 2,
		he3d.gl.viewportHeight / 2 - height / 2,
		width, height);
	...
	// Should do something with this =P
	if (he3d.t.textures[he3d.t.viewer.id].type == 'cubemap')
		return;

	he3d.gl.activeTexture(he3d.gl.TEXTURE0);
	he3d.gl.bindTexture(he3d.gl.TEXTURE_2D, he3d.t.textures[he3d.t.viewer.id].texture);
