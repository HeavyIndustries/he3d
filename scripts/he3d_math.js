...
he3d.m.mat3.identity = function(dest) {
	if (!dest)
...
he3d.m.mat3.multiplyVec3 = function(mat, vec, dest) {
	if (!dest)
		dest = vec;
...
he3d.m.mat4.multiplyVec4 = function(mat, vec, dest) {
	if (!dest)
		dest = vec;
...
he3d.m.quat4.eulerAngleCreate = function(x, y, z, dest) {
	...
	var radiansX = he3d.m.degtorad(x),
		radiansY = he3d.m.degtorad(y),
		radiansZ = he3d.m.degtorad(z),
		cX = Math.cos(radiansX * 0.5),
		cY = Math.cos(radiansY * 0.5),
		cZ = Math.cos(radiansZ * 0.5),
		sX = Math.sin(radiansX * 0.5),
		sY = Math.sin(radiansY * 0.5),
		sZ = Math.sin(radiansZ * 0.5);
...
he3d.m.quat4.slerp = function(quat, quat2, slerp, dest) {
	if (!dest)
		dest = quat;
	var cosHalfTheta = quat[0] * quat2[0] + quat[1] * quat2[1] +
...
//	- Target Bounding Box Dimensions
//
he3d.m.RayvsAABB = function(rs, re, pos, bb) {
	var rayLen = he3d.m.vec3.dist(rs, re);

	// Too far away, early out
	if (he3d.m.vec3.dist(rs, pos) > rayLen)
		return false;

	if ((rs[0] < pos[0] + bb.x.min && re[0] < pos[0] + bb.x.min) ||
		(rs[0] > pos[0] + bb.x.max && re[0] > pos[0] + bb.x.max))
		return false;
	if ((rs[1] < pos[1] + bb.y.min && re[1] < pos[1] + bb.y.min) ||
		(rs[1] > pos[1] + bb.y.max && re[1] > pos[1] + bb.y.max))
		return false;
	if ((rs[2] < pos[2] + bb.z.min && re[2] < pos[2] + bb.z.min) ||
		(rs[2] > pos[2] + bb.z.max && re[2] > pos[2] + bb.z.max))
		return false;
...
he3d.m.dirtycos = function(x) {
	...
	t = t * t;
	y = pc0 + (pc1 * t) + (pc2 * t * t) + (pc3 * t * t * t) +
		(pc4 * t * t * t * t) + (pc5 * t * t * t * t * t);
