...
he3d.console.execute=function(){
	...
	switch(value[0]){
		case 'clearcmdhistory':
			...
			try{localStorage.setItem("consolehistory",
				he3d.console.cmdhistory.join('|c|'));}catch(e){}
		...
		case 'r_fxaa':
			...
			break;
		case 'r_drawfps':
			if(value.length==2&&value[1].length&&!isNaN(value[1])&&value[1]>=0&&value[1]<=2)
	...
	if(!dupe){
		...
		try{localStorage.setItem("consolehistory",he3d.console.cmdhistory.join('|c|'));}catch(e){}
	}
	he3d.console.input.value='';
};
...
he3d.console.initBindings=function(){
	...
	he3d.console.history.addEventListener('DOMMouseScroll',function(e){
		if(e.detail>0) he3d.console.scroll('down');
		else he3d.console.scroll('up');
	},false);
	he3d.console.history.addEventListener('mousewheel',function(e){
		if(e.wheelDelta>0) he3d.console.scroll('up');
		else he3d.console.scroll('down');
	},false);
};

he3d.console.scroll=function(dir){
	if(!he3d.console.open)return;
	if(dir=='up')he3d.console.history.scrollTop-=30;
	else he3d.console.history.scrollTop+=30;
};
